/* -------------------------------------------------------------------------------------------------------
   ...
   --  Перед выполнением CREATE EXTENSION должна быть создана роль:
...
COMMENT ON FUNCTION sepgsql.sepgsql_set_column_label_t (text, text, text) 
   IS '379: Установка нового контекста столбца. Доверенная функция-обёртка над штатным "SECURITY LABEL" 
    ...
    Пример использования:
        SELECT sepgsql.sepgsql_set_column_label_t (''my_sch.t44.f40'', ''system_u:object_r:sepgsql_table_t:s1:c1'');
...
COMMENT ON FUNCTION sepgsql.sepgsql_set_con_t (text) 
   IS '350: Установка контекста сессии. Доверенная функция-обёртка над штатной "sepgsql_setcon()" 
    ...
    Пример использования:
        SELECT * FROM sepgsql_set_con_t (''staff_u:staff_r:staff_t:s1-s3:c0.c15'');
...
COMMENT ON FUNCTION sepgsql.sepgsql_set_db_label_t (text, text, text) 
   IS '436: Установка нового контекста базы. Доверенная функция-обёртка над штатным "SECURITY LABEL" 
    ...
    Пример использования:
        SELECT sepgsql.sepgsql_set_db_label_t (''test_0'', ''system_u:object_r:sepgsql_db_t:s0'');
...
COMMENT ON FUNCTION sepgsql.sepgsql_set_func_label_t (text, text, text) 
   IS '432: Установка нового контекста функции. Доверенная функция-обёртка над штатным "SECURITY LABEL" 
    ...
    Пример использования:
        SELECT * FROM sepgsql.sepgsql_set_func_label_t (''my_sch.f_f2t (p_rec_id int)'', ''system_u:object_r:sepgsql_proc_exec_t:s0-s3:c0.c15'');
...
COMMENT ON FUNCTION sepgsql.sepgsql_set_sch_label_t (text, text, text) 
   IS '436: Установка нового контекста схемы. Доверенная функция-обёртка над штатным "SECURITY LABEL" 
    ...
    Пример использования:
        SELECT sepgsql.sepgsql_set_sch_label_t (''my_sch'', ''system_u:object_r:sepgsql_schema_t:s0'');
...
COMMENT ON FUNCTION sepgsql.sepgsql_set_seq_label_t (text, text, text) 
   IS '436: Установка нового контекста последовательности. Доверенная функция-обёртка над штатным "SECURITY LABEL" 
    ...
    Пример использования:
        SELECT sepgsql.sepgsql_set_seq_label_t (''my_sch.t33_f30_seq'', ''system_u:object_r:sepgsql_seq_t:s3'');
...
COMMENT ON FUNCTION sepgsql.sepgsql_set_table_label_t (text, text, text) 
   IS '379: Установка нового контекста таблицы. Доверенная функция-обёртка над штатным "SECURITY LABEL" 
    ...
    Пример использования:
        SELECT * FROM sepgsql.sepgsql_set_table_label_t (''my_sch.t44'', ''system_u:object_r:sepgsql_table_t:s0-s3:c0.c15'');
...
COMMENT ON FUNCTION sepgsql.sepgsql_set_tuple_label_t (text, int, text) 
   IS '30: Установка контекста безопасности строки таблицы.
       ...
       Пример использования:   
            SELECT * FROM sepgsql.sepgsql_set_tuple_label_t (''bookings.aircrafts''::text, 14, ''staff_u:object_r:sepgsql_tuple_t:s3:c0''::text);
   ';        
...
--
-- SELECT * FROM sepgsql.sepgsql_set_tuple_label_t ('bookings.aircrafts'::text, 14, 'staff_u:object_r:sepgsql_tuple_t:s3:c0'::text);
-- SELECT * FROM sepgsql.sepgsql_set_tuple_label_t ('bookings1.aircrafts'::text, 14, 'staff_u:object_r:sepgsql_tuple_t:s3:c0'::text); 
-- SELECT * FROM sepgsql.sepgsql_set_tuple_label_t ('bookings.aircrafts'::text, 14, 'staff_u:object_r:se2pgsql2_tuple_t:s3:c0'::text); 
--
-- 2020-04-04 Nick
--
...
COMMENT ON FUNCTION sepgsql.sepgsql_set_view_label_t (text, text, text) 
   IS '432: Установка нового контекста представления. Доверенная функция-обёртка над штатным "SECURITY LABEL" 
    ...
    Пример использования:
        SELECT * FROM sepgsql.sepgsql_set_view_label_t (''my_sch.t44'', ''system_u:object_r:sepgsql_view_t:s0-s3:c0.c15'');
   ...
   IS '1163: Сравнение контекстов безопасности клиента и таблицы.
       ...
       Пример использования:   
              SELECT * FROM sepgsql.sepgsql_check_cltb_con (
                    ''staff_u:staff_r:staff_t:s0:c0''
                    ''system_u:object_r:sepgsql_table_t:s0-s3:c0''
                    )
   '; 
--
--
...
COMMENT ON FUNCTION sepgsql.sepgsql_create_tuple_label (text, boolean) 
   ...
   ';  
----------------------------------------------------------------------------
--            SELECT sepgsql_getcon(); -- "system_u:object_r:netlabel_peer_t:s0:c0"
--   2020-04-04 Nick
-- ------------------------------------------------------------
--  Функции типа AUTH
-- -----------------------------------------------------------
...
-- -- 2020-04-04 Nick      
--
CREATE OR REPLACE FUNCTION sepgsql.pg_mls_table_add (
      ...
      ,p_se_user_role  text   = 'system_u:object_r'
...
$$
 ...
 DECLARE 
   ...
   _se_user_role  text   := COALESCE ((btrim (lower(p_se_user_role))), 'system_u:object_r');
...
COMMENT ON FUNCTION sepgsql.pg_mls_table_add (text, text, text[], text, text, text, text) IS 
  ...
  SELECT sepgsql.pg_mls_table_add (
...
--
CREATE OR REPLACE FUNCTION sepgsql.pg_mls_table_remove (
       p_app_sch_name  text -- NOT NULL    --  Имя прикладной схемы, прикладных схем может быть несколько.              
      ,p_table_name    text -- NOT NULL    --  Имя таблицы, одна таблица в одной схеме.
      ,p_section_name  text -- NOT NULL    --  Имя секции с метками  
      ,p_roles         text[] = ARRAY ['sepgsql_role'] --  Роли- участники
      ,p_se_user_role  text   = 'system_u:object_r' -- SE user, SE role
      ,p_table_range   text   = 's0:c0' --  ТАБЛИЦА: Уровень, категория (по умолчанию)
    )
  RETURNS text  
     SECURITY DEFINER 
...
COMMENT ON FUNCTION sepgsql.pg_mls_table_remove (text, text, text, text[], text, text) IS 
      ...
      ,p_section_name  text -- NOT NULL    --  Имя секции с метками  
      ,p_roles         text[] = ARRAY [''sepgsql_role''] --  Роли- участники
      ,p_se_user_role  text   = ''staff_u:object_r''   -- SE user, SE role
      ,p_table_range   text   = ''s0:c0''                --  ТАБЛИЦА: Уровень, категория (по умолчанию)    
 ';
--
-- -----------------------------------------------------------------------------------------------------------------
...
-- ADD/REMOVE
--
GRANT EXECUTE ON FUNCTION sepgsql.pg_mls_table_add (text, text, text[], text, text, text, text) TO sepgsql_role; -- 2020-04-15
GRANT EXECUTE ON FUNCTION sepgsql.pg_mls_table_remove (text, text, text, text[], text, text)    TO sepgsql_role; -- 2020-04-15
